...
public class Field : MonoBehaviour
{
    public static Field Create(int size, int emptySquares)
    {
        ...
        if((size % 2) == 0)
        {
            fieldPosition = new Vector3(0.5f, 0.5f, 0.5f);
        }

        var field = Instantiate(Resources.Load("Prefab/Field") as GameObject, fieldPosition, Quaternion.identity);
    ...
    {
        //first token in left down corner
        ...
        for (int i = 0; i < size; i++)
        {
            for (int j = 0; j < size; j++)
            {
                ...
                else
                {
                    ...
                    {
                        Token newToken = Instantiate(Resources.Load("Prefab/Token"), new Vector3(startPosition.x + i, startPosition.y + j, startPosition.z), Quaternion.identity) as Token;
                    }
                    else
                    {
                        emptySquares--;
